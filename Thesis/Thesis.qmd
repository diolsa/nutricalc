...
Efficient nutrient-solution formulation is fundamental to controlled horticultural production, yet in practice it is often based on fixed recipes or iterative manual adjustment. This thesis addresses that limitation through the developmet of a modular open-source computational framework in R for reproducible nutrient-solution design called NutriCalc.
...
The development of the computational framework followed a structured development and validation pipeline. For the development of the R package devtools was used [@Wickham.2025].

The first step involved designing the underlying data structures. A nutrient matrix was established, in which the rows represent essential nutrients (such as N, P, K, Ca, etc.) and the columns represent fertilizer salts. For readability the input matrix was transposed and converted for the calculation. Each fertilizer was parsed based on its molecular formula into its elemental components, using a database of molar masses and established stoichiometric rules.

Building on this, an R package was be developed. This package includes core functions for parsing chemical formulas and computing molar masses, as well as for converting between different chemical representations such as oxides and elemental forms (e.g., P₂O₅ to P or SO₄ to S). It also include tools for transforming concentration units (e.g., from mmol/L to mg/L) using accurate molar mass calculations. Also bidirectional transformations between compound and elemental quantities, incorporating stoichiometric logic, was integrated.

The central component of the package was be thr implementation of the nutrient solution formulation algorithm, using the nnls::nnls() function to solve the system of equations under non-negativity constraints and giving the difference between desired and obtained solution as absolute and relative error. The framework allows users to assign weights to individual nutrients, enabling exploration of different formulation priorities and trade-offs.

The framework was be validated through a series of use cases. These included reproducing historical and crop-specific nutrient solutions, comparing the computed results with values from the literature and collecting some crop and author specific nutrient solutions in a base library. Nutrient distribution of anions and cations across different solutions is be visualized e.g. by ternary plot for cations (K+, Ca2+, Mg+) and anions (NO₃⁻, SO₄²⁻, PO₄3⁻) classify systematically different nutrient combinations as done by @Steiner.1961.
...
In his work @Hewitt.1966 collected a majority of the formulation used in the first half of the 20 Century.

In this work a set of horticultural recipes form @Sonneveld.1994 where selected. Those xx recipes where divided into the three categories and "Olericulture", "Floriculture" and "Fruticulture". @Sonneveld.1994 divided the recipes in three Categories "A", "B" an "C", to reflect the of practical and experimental fitness of the reported formulation for the crop. in @tbl-crop-recipes-long-landscape_units the respective formulations are shown.
...
tbl <- knitr::kable(
  ...
  caption = "Solver-derived pH, ionic strength, total charge equivalents (TCE), electrical conductivity (EC) and osmotic pressure for crop-specific formulations (salts-only; water = 0; KS 4.3 = 0; CO2(aq) = 0). EC sonneveld and reportet EC for comparison.",
  escape = TRUE
) |>
  kable_styling(latex_options = "repeat_header", font_size = 7) |>
  collapse_rows(columns = 1, latex_hline = "major", valign = "top") |>
...
### Case study 1 — Experimental scientist: classical solution reconstruction under restricted salt availability (deionized water) under different EC.
...
### Case study 2 — Horticultural technician: tomato nutrient formulation using Berlin-Dahlem water with acid adjustment
...
![Screenshot of the fertilizer salt composition calculated in the NutriCalc Shiny app for a tomato nutrient solution in rockwool using tap water from Berlin postal code 14195. The formulation is shown separately for A-BAK, B-BAK, and micronutrients stock solutions. Fertilizer requirements are reported for the preparation of 500 L nutrient solution and the amounts for 18 L of a 50-fold concentrated stock solution.](Thesis_screenshots/tomato2final.png){#fig-tomato-salt-composition width="15cm"}
...
### Case study 3 — Research scientist: nutrient formulation for a novel halophyte (Salicornia) using mixed water sources
...
# Summary (possibly in German and English)
...
# Declaration of Indepedence {.unnumbered}
